...
import serial
ser = serial.Serial("COM3", 19200)  #khởi tạo cho mình 1 cổng giao tiếp UART, lưu vào biến Ser
def SerialWrite(s):   #gửi giá trị 's' xuống arduino thông qua UART
    ser.write(s.encode())
...
g_Image_Grabbing_Timer = 1500  # unit : second
g_isStop = 0
...
def onGetFrameEx(frame, userInfo):
    ...
    # --- end if ---
    frame = cv2.resize(cvImage, dsize=None, fx=0.5, fy=0.5)
    ...
    for label in labels:
        # if (label.find("apple")==0): SerialWrite('i')
        # elif (label.find("banana")==0): SerialWrite('j')
        # elif (label.find("orange")==0): SerialWrite('k')

        if (label.find("tac") == 0): SerialWrite('t')
        if (label.find("nhan") == 0): SerialWrite('n')
        if (label.find("cachua") == 0): SerialWrite('c')
